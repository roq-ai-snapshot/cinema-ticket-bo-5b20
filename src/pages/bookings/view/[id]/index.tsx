...
function BookingViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; showing: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'showing'],
          id,
        },
        'booking',
      ),
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Booking Time"
                  text={data?.booking_time ? format(data?.booking_time, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Number Of Tickets" text={data?.number_of_tickets} />
                ...
                {hasAccess('showing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Showing"
                    text={
                      <Link as={NextLink} href={`/showings/view/${data?.showing?.id}`}>
                        {data?.showing?.showing_time + ''}
                      </Link>
                    }
                  />
                )}
