...
import { useRoqPlatformUserProfiles, useTechnicalSupportCount, useShowingCount, useBookingCount } from 'lib/roq';

import { MovieListPage as ShowCaseList } from 'pages/movies';

function DashboardPage() {
  const { session } = useSession();

  const technicalSupportStat = useTechnicalSupportCount();

  const showingStat = useShowingCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: technicalSupportStat?.data,
      isLoading: technicalSupportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Showings',
      result: showingStat?.data,
      isLoading: showingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`movies`} passHref legacyBehavior>
                  <Button
