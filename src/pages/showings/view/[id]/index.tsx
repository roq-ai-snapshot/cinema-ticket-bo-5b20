...
import { useShowingFindFirst, useRoqClient } from 'lib/roq';

import { ShowingInterface } from 'interfaces/showing';
import { BookingListPage } from 'pages/bookings';

function ShowingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { cinema: boolean; movie: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useShowingFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('showing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/showings/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Showing Time"
                  text={data?.showing_time ? format(data?.showing_time, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('cinema', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Cinema"
                    text={
                      <Link as={NextLink} href={`/cinemas/view/${data?.cinema?.id}`}>
                        {data?.cinema?.name + ''}
                      </Link>
                    }
                  />
